...
export default function TutorSimProjectDetail() {
  const navigate = useNavigate();

  const { data: user } = useQuery({ queryKey: ['current-user'], queryFn: () => base44.auth.me() });

  // Verify tutor is assigned to this project
  const { data: tutorAssignments = [], isLoading: checkingAccess } = useQuery({
    queryKey: ['tutor-sim-assignments', user?.id],
    queryFn: () => base44.entities.SimProjectTutorAssignment.filter({ tutor_user_id: user.id }),
    enabled: !!user?.id,
  });
  ...
  const { data: project, isLoading: loadingProject } = useQuery({
    queryKey: ['sim-project', PROJECT_ID],
    queryFn: () => base44.entities.SimProject.filter({ id: PROJECT_ID }).then(r => r[0]),
    enabled: !!PROJECT_ID && isAssigned,
  });

  const { data: phases = [] } = useQuery({
    queryKey: ['sim-phases', PROJECT_ID],
    queryFn: () => base44.entities.SimProjectPhase.filter({ project_id: PROJECT_ID }),
    enabled: !!PROJECT_ID && isAssigned,
  });

  const { data: tasks = [] } = useQuery({
    queryKey: ['sim-tasks', PROJECT_ID],
    queryFn: () => base44.entities.SimProjectTask.filter({ project_id: PROJECT_ID }),
    enabled: !!PROJECT_ID && isAssigned,
  });

  // Only enrollments assigned to THIS tutor
  const { data: enrollments = [], isLoading: loadingEnrollments } = useQuery({
    queryKey: ['sim-enrollments-tutor', PROJECT_ID, user?.id],
    queryFn: () => base44.entities.SimProjectEnrollment.filter({ project_id: PROJECT_ID, reviewer_tutor_id: user.id }),
  ...
  const { data: submissions = [] } = useQuery({
    queryKey: ['sim-submissions-project', PROJECT_ID],
    queryFn: () => base44.entities.SimTaskSubmission.filter({ project_id: PROJECT_ID }),
    enabled: !!PROJECT_ID && isAssigned,
  });
  ...
  if (!PROJECT_ID) {
    navigate(createPageUrl('TutorSimProjects'));
    return null;
  }
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-6">
        ...
        >
          ...
          <div className="px-8 py-8">
            ...
            <button
              onClick={() => navigate(createPageUrl('TutorSimProjects'))}
  ...
  // Merge enrollment with user info
