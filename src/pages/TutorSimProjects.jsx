...
export default function TutorSimProjects() {
  ...
  const { data: allProjects = [], isLoading: loadingProjects } = useQuery({
    queryKey: ['all-sim-projects'],
    queryFn: () => base44.entities.SimProject.list(),
    enabled: assignedProjectIds.length > 0,
  });

  const { data: allEnrollments = [] } = useQuery({
    queryKey: ['all-sim-enrollments'],
      ...
      <div className="max-w-5xl mx-auto px-8 py-8 space-y-4">
        {myProjects.map((project, i) => {
          ...
          return (
            ...
            >
              <Link
                to={createPageUrl(`TutorSimProjectDetail?id=${project.id}`)}
