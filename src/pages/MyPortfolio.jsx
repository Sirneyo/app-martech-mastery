...
export default function MyPortfolio() {
  ...
  const hasPassed = false; // Set to true when student passes the exam
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-8">
      ...
      <motion.div 
        ...
        className="mb-8"
      >
        <h2 className="text-2xl font-bold text-slate-900 mb-4">My Certification</h2>
        ...
        <div className={`bg-white rounded-xl p-6 border shadow-sm ${
          ...
          <div className="flex items-start gap-4 mb-4">
            <div className={`w-16 h-16 rounded-xl flex items-center justify-center ${
              hasPassed ? 'bg-gradient-to-br from-amber-500 to-orange-500' : 
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <h3 className="text-xl font-bold text-slate-900">MarTech Mastery Certification Exam</h3>
                {!isCertificationUnlocked && (
                  <Badge className="bg-slate-100 text-slate-400">Locked</Badge>
                )}
                {isCertificationUnlocked && !hasPassed && (
                  <Badge className="bg-violet-100 text-violet-700">Available</Badge>
                )}
                {hasPassed && (
              ...
              <p className="text-slate-600 mb-3">
                This happens in week 8 of the program where every student must pass the MarTech Mastery 
                Certification Exam. After passing, you'll receive a certification that confirms your status 
                as a <strong>Certified Marketing Operations Professional</strong>.
              </p>
              {!isCertificationUnlocked && (
                <p className="text-sm text-slate-400 flex items-center gap-1">
                  <Lock className="w-3 h-3" /> Unlocks at Week 8
                </p>
              )}
              {isCertificationUnlocked && !hasPassed && (
          ...
          {hasPassed && (
            <motion.div
              ...
              className="mt-6 bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200 rounded-xl p-8"
            >
              <div className="text-center">
                <div className="w-24 h-24 mx-auto bg-gradient-to-br from-amber-500 to-orange-500 rounded-full flex items-center justify-center mb-4 shadow-lg">
                  <Award className="w-12 h-12 text-white" />
                </div>
                <h4 className="text-2xl font-bold text-slate-900 mb-2">
                  Certified Marketing Operations Professional
                </h4>
                <p className="text-slate-600 mb-4">MarTech Mastery Certification</p>
                <div className="flex items-center justify-center gap-2 text-sm text-slate-500 mb-6">
                  <span>Issued: {new Date().toLocaleDateString()}</span>
                  <span>•</span>
                  <span>Credential ID: MM-{Math.floor(100000 + Math.random() * 900000)}</span>
                </div>
