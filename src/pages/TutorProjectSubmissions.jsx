...
export default function TutorProjectSubmissions() {
  ...
  const { data: submissions = [] } = useQuery({
    queryKey: ['project-submissions', cohortIds, statusFilter],
    queryFn: async () => {
      ...
      const allSubmissions = await base44.entities.Submission.filter({ submission_kind: 'project' });
  ...
  const getTemplateName = (templateId) => {
    ...
    return template?.title || 'Unknown Project';
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-8">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Project Submissions</h1>
            <p className="text-slate-500 mt-1">Review and grade student projects</p>
          </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-bold text-slate-900">{getTemplateName(submission.project_template_id)}</h3>
                      <Badge className={
