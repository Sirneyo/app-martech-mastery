...
export default function TaskDetailPanel({ task, submission, onClose, userId, enrollmentId, projectId, phases }) {
  ...
  const updateSubmission = useMutation({
    mutationFn: async (data) => {
      if (submission?.id) {
        await base44.entities.SimTaskSubmission.update(submission.id, data);
      } else {
  ...
  const addComment = useMutation({
    mutationFn: async (text) => {
      ...
      if (!subId) {
        const sub = await base44.entities.SimTaskSubmission.create({
          project_id: projectId,
          task_id: task.id,
          enrollment_id: enrollmentId,
          student_user_id: userId,
      ...
      await base44.entities.SimTaskComment.create({
