...
import {
  ...
  GripVertical, ChevronDown, ChevronRight, CheckSquare, Square,
  MessageSquare, FileText, Link as LinkIcon, Upload
} from 'lucide-react';
...
export default function KanbanBoard({ tasks, phases, submissions, enrollmentId, userId, projectId, onTaskClick }) {
  ...
  // Sort within column by sort_order
  ...
  const updateSubmissionStatus = useMutation({
    mutationFn: async ({ taskId, newStatus }) => {
      ...
      if (existing) {
        await base44.entities.SimTaskSubmission.update(existing.id, { status: newStatus });
      } else {
        await base44.entities.SimTaskSubmission.create({
  ...
  const onDragEnd = useCallback((result) => {
    ...
    // Prevent moving approved tasks unless it's intentional (allow back to in_progress)
